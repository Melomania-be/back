...
export default class ContactsController {
  ...
  async getOne({ params, bouncer }: HttpContext) {
    ...
    return await Contact.query()
      ...
      .preload('participants', (query) => {
        query.preload('project').preload('section', (subQuery) => {
  ...
  async mergeContacts(ctx: HttpContext) {
    ...
    if (!data.contactId1 && !data.contactId2) return ctx.response.status(400).send('No contact ids provided')
    if (data.contactId1 === data.contactId2) return ctx.response.status(400).send('Cannot merge a contact with itself')

    const contact1 = await Contact.query().preload('instruments').preload('lists').preload('participants').preload('projects').where('id', data.contactId1).firstOrFail()
    const contact2 = await Contact.query().preload('instruments').preload('lists').preload('participants').preload('projects').where('id', data.contactId2).firstOrFail()
    ...
    await contact1.related('lists').sync(contact1.lists.concat(contact2.lists).map((list) => list.id), false)
    await contact1.related('projects').sync(contact1.projects.concat(contact2.projects).map((project) => project.id), false)
    ...
    for (let participant2 of participants2) {
      if (projectIds1.has(participant2.project_id)) {
        ...
        if (participant1) await participant1.delete()
      }
      participant2.contact_id = contact1.id
      await participant2.save()
    }

    await contact1.related('instruments').sync(contact1.instruments.concat(contact2.instruments).map((instrument) => instrument.id), false)
  ...
  async createOrUpdate(ctx: HttpContext) {
    ...
    if (!data.id) return await Contact.create({ ...data, validated: true })

    const contact = await Contact.updateOrCreate({ id: data.id }, { ...data, validated: true })

    if (data.instruments) {
      let toSync = Object.assign({}, ...data.instruments.map((instrument) => {
        return { [instrument.id]: { proficiency_level: instrument.pivot_proficiency_level } }
      }))
  ...
  async create(ctx: HttpContext) {
    await (ctx.bouncer as any).authorize('adminRights')
    const data = await ctx.request.validateUsing(createContactValidator)
    const existing = await Contact.query().where('firstname', data.first_name).andWhere('lastname', data.last_name).first()
  ...
  async getValidation(ctx: HttpContext) {
    ...
    return await simpleFilter(
      ctx, baseQuery, ['first_name', 'last_name', 'email', 'comments', 'messenger', 'phone'],
      [{ relationColumns: ['family', 'name'], relationName: 'instruments' }]
    )
  }

  // Action PUBLIQUE (désinscription par email, pas de bouncer requis)
