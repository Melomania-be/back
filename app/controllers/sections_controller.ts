...
export default class SectionsController {
  ...
  async createOrUpdate(ctx: HttpContext) {
    ...
    if (data.id === undefined) {
      ...
      for (let instrument of data.instruments) if (!instrument.id) return ctx.response.status(400).send('save instrument first')
      return section.related('instruments').sync(data.instruments.map((i) => i.id!))
    } else {
      let section = await Section.find(data.id)
      if (!section) return ctx.response.status(404).send('not found')
      section.name = data.name; section.size = data.size; await section.save()
      for (let instrument of data.instruments) if (!instrument.id) return ctx.response.status(400).send('save instrument first')
      return section.related('instruments').sync(data.instruments.map((i) => i.id!))
    }
  }
